    ...
    public class CountFuel
    {
        ...
        public void CountFuelHour(string date)
        {
            ...
            try
            {
                for (int i = hourMIn; i <= hourMax; i++)
                {
                    ...
                    List<Dictionary<string, string>> list = mysql.MultipleSelect(sql, new List<string>() { "vid", "type" });
                    if (list != null)
                    {
                        foreach (var dic in list)
                        {
                            ...
                            List<double> fuel = mysql.MultipleSelect(sql, "fuel");
                            if (fuel != null)
                            {
                                //计算当前时间段内当前车辆加油与耗油
        ...
        public void CountFuelDay(string date)
        {
            ...
            try
            {
                ...
                List<Dictionary<string, string>> list = mysql.MultipleSelect(sql, new List<string>() { "useFuel", "inject", "unfuel", "vid", "type" });
                if (list != null)
                {
        ...
        public void CheckFuel(object source, System.Timers.ElapsedEventArgs e)
        {
            ...
            try
            {
                ...
                List<string> VehicleList = mysql.MultipleSelect(sql, "vid", "");
                if (VehicleList==null) {
                    return;
                }
                foreach (var item in VehicleList)
                {
                    ...
                    List<Dictionary<string, string>> max = mysql.MultipleSelect(sql, new List<string>() { "REAL_FUEL", "ID" });
                    ...
                    List<Dictionary<string, string>> min = mysql.MultipleSelect(sql, new List<string>() { "REAL_FUEL", "ID" });
                    ...
                    if (max != null && min != null)
                    {
                        ...
                        if (data > Fuel && int.Parse(maxId) < int.Parse(minId))//判断油量最大值是否在前
                        {
                            ...
                            if (mysql.GetCount(sql) == 0)
                            {
                                ...
                                List<Dictionary<string, string>> result = mysql.MultipleSelect(sql, new List<string>() { "vid", "driver", "type" });
